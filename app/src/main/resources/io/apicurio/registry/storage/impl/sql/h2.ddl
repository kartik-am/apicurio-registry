...
CREATE TABLE markdown (tenantId VARCHAR(128) NOT NULL, groupId VARCHAR(512) NOT NULL, artifactId VARCHAR(512) NOT NULL, version VARCHAR(256) NOT NULL, content BYTEA NOT NULL);
ALTER TABLE markdown ADD PRIMARY KEY (tenantId, groupId, artifactId, version);
ALTER TABLE markdown ADD CONSTRAINT FK_markdown_1 FOREIGN KEY (tenantId, groupId, artifactId, version) REFERENCES versions(tenantId, groupId, artifactId, version) ON DELETE CASCADE;
